...
function buildSignatureSessionKey(
  ...
  const modelKey =
    typeof model === "string" && model.trim() ? model.toLowerCase() : "unknown";
  const projectPart =
...
function extractConversationSeedFromMessages(messages: any[]): string {
  ...
  const fallbackUserText =
    !userText && lastUser ? extractTextFromContent(lastUser.content) : "";
...
function extractConversationSeedFromContents(contents: any[]): string {
  ...
  const lastUser = users.length > 0 ? users[users.length - 1] : undefined;
  const primaryUser =
    firstUser && Array.isArray(firstUser.parts)
...
function resolveConversationKeyFromRequests(
...
function injectDebugThinking(response: unknown, debugText: string): unknown {
  ...
  if (Array.isArray(resp.content)) {
    const content = [
      { type: "thinking", thinking: debugText },
      ...resp.content,
    ];
    return { ...resp, content };
  }
...
function stripInjectedDebugFromRequestPayload(
  ...
  if (Array.isArray(anyPayload.contents)) {
    anyPayload.contents = anyPayload.contents.map((content: any) => {
      ...
      if (Array.isArray(content.parts)) {
        return {
          ...content,
          parts: stripInjectedDebugFromParts(content.parts),
        };
      }
...
function sanitizeRequestPayloadForAntigravity(
  ...
  if (Array.isArray(anyPayload.contents)) {
    anyPayload.contents = anyPayload.contents
        ...
        if (restore?.sessionKey) {
          for (const candidatePart of rawParts) {
            if (
              ...
              (candidatePart as any).thought !== true
            ) {
              continue;
            }
            ...
            const cached = restore.getSignature(
              restore.sessionKey,
              thoughtText,
            );
            if (
              cached &&
              cached.length >= MIN_SIGNATURE_LENGTH &&
              cached !== SKIP_THOUGHT_SIGNATURE
            ) {
      ...
      .filter(
        (content: unknown): content is Record<string, unknown> =>
          content !== null,
      );
  }
...
function ensureThinkingBeforeToolUseInContents(
  ...
  return contents.map((content: any) => {
    ...
    if (!lastThinking) {
      ...
      log.debug(
        "Stripping thinking from tool_use content (no valid cached signature)",
        { signatureSessionKey },
      );
...
function ensureThinkingBeforeToolUseInMessages(
  ...
  return messages.map((message: any) => {
    ...
    const hasSignedThinking = thinkingBlocks.some((block) =>
      hasSignedThinkingPart(block, signatureSessionKey),
    );
    ...
    if (!lastThinking) {
      ...
      log.debug("Injecting sentinel signature (cache miss)", {
        signatureSessionKey,
      });
      const sentinelBlock = {
        type: "thinking",
...
function ensureDefaultGemini3ThinkingLevel(
  ...
  if (
    !effectiveModel.toLowerCase().includes("gemini-3") ||
...
export function isGenerativeLanguageRequest(input: RequestInfo): boolean {
  try {
    return (
      new URL(requestInfoUrl(input)).hostname ===
...
export function prepareAntigravityRequest(
  ...
  const enableClaudePromptAutoCaching =
    options?.claudePromptAutoCaching ?? false;
    ...
    try {
      ...
      if (isWrapped) {
        ...
        if (effectiveModel.toLowerCase().includes("gemini-3")) {
          for (const req of requestObjects) {
            const variantConfig = extractVariantThinkingConfig(
              (req as any).providerOptions as
                | Record<string, unknown>
                | undefined,
              (req as any).generationConfig as
                | Record<string, unknown>
                | undefined,
            );
        ...
        const conversationKey =
          resolveConversationKeyFromRequests(requestObjects);
          ...
          const hasToolUse = requestObjects.some(
            (req) =>
              (Array.isArray((req as any).contents) &&
                hasToolUseInContents((req as any).contents)) ||
              (Array.isArray((req as any).messages) &&
                hasToolUseInMessages((req as any).messages)),
          );
        ...
        const rawGenerationConfig = requestPayload.generationConfig as
          | Record<string, unknown>
          | undefined;
        const extraBody = requestPayload.extra_body as
          | Record<string, unknown>
          | undefined;
        ...
        if (isClaude) {
          ...
          if (
            typeof requestPayload.toolConfig === "object" &&
            requestPayload.toolConfig !== null
          ) {
            const toolConfig = requestPayload.toolConfig as Record<
              string,
              unknown
            >;
        ...
        const isClaudeSonnetNonThinking =
          lowerEffective === "claude-sonnet-4-6";
        const effectiveUserThinkingConfig =
          isClaudeSonnetNonThinking || isImageModel
            ? undefined
            : userThinkingConfig;
        ...
        if (isImageModel) {
          ...
          const generationConfig = (rawGenerationConfig ?? {}) as Record<
            string,
            unknown
          >;
          ...
          const normalizedThinking =
            normalizeThinkingConfig(finalThinkingConfig);
          ...
          if (isClaude) {
            ...
            (requestPayload.tools as any[]).forEach((tool: any) => {
              ...
              if (
                ...
                pushDeclaration(
                  tool.function ?? tool.custom ?? tool,
                  "function/custom",
                );
                return;
              }
        ...
        if (isClaude && Array.isArray(requestPayload.contents)) {
          ...
          requestPayload.contents = (requestPayload.contents as any[]).map(
            (content: any) => {
              if (!content || !Array.isArray(content.parts)) {
                return content;
              }
          ...
          requestPayload.contents = fixToolResponseGrouping(
            requestPayload.contents as any[],
          );
        }
        ...
        sanitizeRequestPayloadForAntigravity(
          ...
          restoreGeminiThoughtSignatures
            ? {
                ...
                getSignature: getCachedSignature,
              }
            : undefined,
        );
        ...
        if (headerStyle === "antigravity") {
          ...
          if (
            ...
            typeof existingSystemInstruction === "object"
          ) {
...
export function buildThinkingWarmupBody(
  ...
  const updateRequest = (req: Record<string, unknown>) => {
    ...
    const generationConfig = (req.generationConfig ?? {}) as Record<
      string,
      unknown
    >;
    generationConfig.thinkingConfig = {
...
export async function transformAntigravityResponse(
  ...
  try {
    ...
    const usageFromSse =
      streaming && isEventStreamResponse
